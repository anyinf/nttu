    ...
    with st.form("medical_record_form", clear_on_submit=True):
        ...
        if submit_med:
            if med_result:
                ...
                updated_med_df = pd.concat([df_medical, pd.DataFrame([new_med_row])], ignore_index=True)
                updated_med_df = updated_med_df.sort_values(by="就醫時間", ascending=False).reset_index(drop=True)
                
                st.session_state["medical_df"] = updated_med_df
                save_data(updated_med_df, MEDICAL_FILE)
