...
# ================= 功能二：新增就醫紀錄 =================
    ...
    with st.form("medical_record_form", clear_on_submit=True):
        ...
        if submit_med:
            if med_result:
                img_path = None # 改為 None，方便 ImageColumn 辨識空值
    ...
    if not df_medical.empty:
        st.info("💡 提示：您可以直接點擊表格中的【圖片路徑】照片，即可放大觀看完整就醫證明！")
        
        # 使用 st.data_editor 搭配 ImageColumn 配置，完美將路徑轉為可點擊放大的圖片
        st.data_editor(
            df_medical,
            column_config={
                "圖片路徑": st.column_config.ImageColumn(
                    "就醫證明照片", 
                    help="點擊圖片可以放大預覽",
                    width="medium"
                )
            },
            disabled=True, # 設為唯讀，防止在表格內誤刪文字
