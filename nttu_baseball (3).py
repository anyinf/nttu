...
if "records_df" not in st.session_state:
    st.session_state["records_df"] = load_data(DATA_FILE, ["ID", "球員姓名", "受傷部位與敘述", "發生日期", "目狀態"])
...
# ================= 功能二：新增就醫紀錄 =================
    ...
    with st.form("medical_record_form", clear_on_submit=True):
        ...
        if submit_med:
            if med_result:
                img_path = None # 改為 None，方便後續辨識空值
    ...
    if not df_medical.empty:
        ...
        # 額外提供一個按鈕列（因為 st.dataframe 不支援內嵌按鈕）
        st.write("---")
        st.caption("👈 點擊左側「查看/點開照片」按鈕，即可在表格下方放大觀看完整證明。")
            ...
            with st.container():
                ...
                with btn_cols[0]:
                    ...
                    if img_path and img_path != 'None' and os.path.exists(img_path):
                        ...
                        st.caption(" ( None ) ")
                with btn_cols[1]:
                    st.caption(f"{row['就醫時間']} - {row['姓名']}: {row['診斷結果'][:30]}...")
        ...
        if st.session_state["current_image"]:
            ...
            try:
                ...
                st.image(img, caption="已上傳的照片", use_column_width=True)
            except Exception as e:
                st.error(f"無法開啟圖片: {e}")
            # 用一個按鈕來清空顯示
