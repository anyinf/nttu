...
# --- 初始化 Session State (確保網頁重整、切換選單時資料不丟失) ---
if "records_df" not in st.session_state:
    st.session_state["records_df"] = load_data(DATA_FILE, ["ID", "球員姓名", "受傷部位與敘述", "發生日期", "目前狀態"])

if "medical_df" not in st.session_state:
    st.session_state["medical_df"] = load_data(MEDICAL_FILE, ["姓名", "就醫時間", "診斷結果", "圖片路徑"])
    ...
    with st.form("add_record_form", clear_on_submit=True):
        ...
        if submit_btn:
            if injury_desc:
                ...
                st.session_state["records_df"] = pd.concat([df_records, pd.DataFrame([new_row])], ignore_index=True)
                save_data(st.session_state["records_df"], DATA_FILE)
...
# ================= 功能二：新增就醫紀錄 =================
    ...
    with st.form("medical_record_form", clear_on_submit=True):
        ...
        if submit_med:
            if med_result:
                ...
                df_medical = st.session_state["medical_df"]
                st.session_state["medical_df"] = pd.concat([df_medical, pd.DataFrame([new_med_row])], ignore_index=True)
                save_data(st.session_state["medical_df"], MEDICAL_FILE)
                
                st.session_state["med_success"] = f"已成功記錄 {med_name} 的就醫資料！"
                st.rerun()
            else:
                st.error("請填寫診斷結果。")

    if "med_success" in st.session_state:
        st.success(st.session_state["med_success"])
        del st.session_state["med_success"]
    ...
    if not df_medical.empty:
        ...
        # ---- 🔥 新增：就醫紀錄刪除區塊 ----
        st.divider()
        st.subheader("⚙️ 就醫紀錄管理 (刪除紀錄)")
        
        # 建立一個供下拉選單辨識的清單清單索引
        med_options = list(df_medical.index)
        record_to_delete = st.selectbox(
            "選擇要刪除的就醫紀錄：",
            options=med_options,
            format_func=lambda x: f"[{df_medical.loc[x, '就醫時間']}] {df_medical.loc[x, '姓名']} - {str(df_medical.loc[x, '診斷結果'])[:20]}..."
        )
        
        # 安全刪除與二次確認
        with st.popover("❌ 刪除此筆就醫紀錄"):
            st.warning("確定要刪除這筆就醫紀錄嗎？刪除後對應的照片檔案也會從硬碟中移除且無法復原。")
            confirm_delete = st.button("確認完全刪除紀錄與照片", type="primary", key="btn_confirm_med_del")
            
            if confirm_delete:
                # 獲取要刪除的圖片路徑
                target_img = df_medical.loc[record_to_delete, "圖片路徑"]
                
                # 如果有實體圖片檔案存在，將其從硬碟刪除
                if target_img and str(target_img) != 'None' and os.path.exists(str(target_img)):
                    try:
                        os.remove(str(target_img))
                    except Exception as e:
                        st.sidebar.error(f"圖片實體檔案刪除失敗: {e}")
                
                # 如果目前看照片看的就是這張，關閉它
                if st.session_state["current_image"] == target_img:
                    st.session_state["current_image"] = None
                
                # 從 DataFrame 移除該筆資料
                df_medical = df_medical.drop(record_to_delete).reset_index(drop=True)
                
                # 儲存回 Session 與 CSV 檔案
                st.session_state["medical_df"] = df_medical
                save_data(df_medical, MEDICAL_FILE)
                
                st.toast("就醫紀錄與相關照片已成功刪除！")
                st.rerun()
